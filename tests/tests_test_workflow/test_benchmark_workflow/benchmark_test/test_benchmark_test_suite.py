...
class TestBenchmarkTestSuite(unittest.TestCase):
    def setUp(self, **kwargs: Any) -> None:
        ...
        self.benchmark_test_suite = BenchmarkTestSuite(endpoint=self.endpoint, security=False, distribution_version='2.3.0', args=self.args, password=get_password('2.3.0'))
    ...
    def test_execute_security_enabled_version_212_or_greater(self) -> None:
        benchmark_test_suite = BenchmarkTestSuite(endpoint=self.endpoint, security=True, distribution_version='2.12.0', args=self.args, password=get_password('2.12.0'))
        with patch("subprocess.check_call") as mock_check_call:
    ...
    def test_execute_security_enabled(self) -> None:
        benchmark_test_suite = BenchmarkTestSuite(endpoint=self.endpoint, security=True, distribution_version='2.3.0', args=self.args, password=get_password('2.3.0'))
        with patch("subprocess.check_call") as mock_check_call:
            benchmark_test_suite.execute()
            self.assertEqual(mock_check_call.call_count, 1)
            self.assertEqual(benchmark_test_suite.command,
