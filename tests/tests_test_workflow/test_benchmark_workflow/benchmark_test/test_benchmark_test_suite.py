...
class TestBenchmarkTestSuite(unittest.TestCase):
    ...
    def test_execute_default(self, mock_check_call: Mock) -> None:
        ...
        with patch('test_workflow.benchmark_test.benchmark_test_suite.TemporaryDirectory') as mock_temp_directory:
            ...
            mock_check_call.assert_called_with(
                f"docker cp docker-container-{test_suite.args.stack_suffix}:opensearch-benchmark/. /mock/temp/dir", cwd=os.getcwd(), shell=True)
            mock_convert.assert_called_once_with('/mock/test_execution.json')

            self.assertEqual(mock_check_call.call_count, 2)
            self.assertEqual(test_suite.command, f'docker run --name docker-container-{test_suite.args.stack_suffix} opensearchproject/opensearch-benchmark:latest execute-test --workload=nyc_taxis --pipeline=benchmark-only --target-hosts=abc.com:80 --client-options="timeout:300"')
    ...
    def test_execute_security_enabled_version_212_or_greater(self, mock_convert: Mock, mock_check_call: Mock) -> None:
        ...
        with patch('test_workflow.benchmark_test.benchmark_test_suite.TemporaryDirectory') as mock_temp_directory:
            ...
            self.assertEqual(test_suite.command,
                             f'docker run --name docker-container-{test_suite.args.stack_suffix} opensearchproject/opensearch-benchmark:latest execute-test --workload=nyc_taxis --pipeline=benchmark-only --target-hosts=abc.com:443 --client-options="timeout:300,use_ssl:true,verify_certs:false,basic_auth_user:\'admin\',basic_auth_password:\'myStrongPassword123!\'"')
    ...
    @patch('test_workflow.benchmark_test.benchmark_test_suite.subprocess.check_call')
    @patch('test_workflow.benchmark_test.benchmark_test_suite.BenchmarkTestSuite.convert')
    def test_execute_with_all_benchmark_optional_params(self, mock_convert: Mock, mock_check_call: Mock) -> None:
        mock_check_call.return_value = 0
        self.args.insecure = True
        TestBenchmarkTestSuite.setUp(self, config="/home/test/benchmark.ini", tags="key1:value1,key2:value2",
                                     workload_params="{\"number_of_replicas\":\"1\"}", test_procedure="test-proc1,test-proc2",
    ...
    def test_convert(self, mock_get_terminal_size, mock_logging_info, mock_open, mock_json_load, mock_read_csv,
                     mock_json_normalize):
        ...
        test_suite = BenchmarkTestSuite("abc.com:80", False, self.args, "")
        test_suite.convert("mock_results.json")

        mock_open.assert_called_once_with("mock_results.json")
