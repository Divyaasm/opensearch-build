...
class TestValidation(unittest.TestCase):
    ...
    def test_install_native_plugin(self, mock_temporary_directory: Mock, mock_validation_args: Mock, mock_plugin_list: Mock, mock_execute: Mock) -> None:
        ...
        validate_tar.install_native_plugin("/tmp/opensearch")
        mock_execute.assert_has_calls(
            [
                call(f'.{os.sep}opensearch-plugin install --batch discovery-ec2', "/tmp/opensearch/bin"),
                call(f'.{os.sep}opensearch-plugin install --batch repository-s3', "/tmp/opensearch/bin")
            ]
        )
