...
class TestValidation(unittest.TestCase):
    ...
    @patch('system.temporary_directory.TemporaryDirectory')
    def test_install_native_plugin_production(self, mock_temporary_directory: Mock, mock_validation_args: Mock, mock_plugin_list: Mock, mock_execute: Mock) -> None:
    ...
    def test_install_native_plugin_staging(self, mock_temporary_directory: Mock, mock_file: Mock,
                                           mock_validation_args: Mock, mock_plugin_list: Mock, mock_execute: Mock, mock_get: Mock) -> None:
        mock_response = Mock()
        ...
        mock_execute.side_effect = lambda *args, **kwargs: (0, "stdout_output", "stderr_output")
        validate_tar = ValidateTar(mock_validation_args.return_value, mock_temporary_directory.return_value)
        validate_tar.install_native_plugin(os.path.join("tmp", "trytytyuit", "opensearch"), ["analysis-icu", "analysis-nori"])
    ...
    @patch('system.temporary_directory.TemporaryDirectory')
    @patch("builtins.open", new_callable=mock_open)
    def test_install_native_plugin_exception(self, mock_file: Mock, mock_temporary_directory: Mock, mock_validation_args: Mock,
