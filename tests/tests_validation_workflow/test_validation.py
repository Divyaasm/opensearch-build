...
class TestValidation(unittest.TestCase):
    ...
    @patch.object(Validation, "download_and_install_native_plugin_zip")
    @patch('validation_workflow.validation.ValidationArgs')
    @patch('system.temporary_directory.TemporaryDirectory')
    def test_install_native_plugin(self, mock_temporary_directory: Mock, mock_validation_args: Mock, mock_download_install_zip: Mock, mock_plugin_list: Mock, mock_execute: Mock) -> None:
        ...
        validate_tar.install_native_plugin(os.path.join("tmp", "trytytyuit"), ["discovery-ec2", "repository-s3"])
        mock_download_install_zip.assert_called()
    ...
    @patch('validation_workflow.validation.ValidationArgs')
    @patch('system.temporary_directory.TemporaryDirectory')
    def test_install_native_plugin_exception(self, mock_temporary_directory: Mock, mock_validation_args: Mock,
                                             mock_download_install_zip: Mock, mock_plugin_list: Mock, mock_execute: Mock) -> None:
        mock_plugin_list.return_value = ["discovery-ec2", "repository-s3"]
        ...
        self.assertEqual(str(e.exception),
                         "Unable to install native plugin: Command './opensearch-plugin install --batch discovery-ec2', tmp/trytytyuit/bin' returned non-zero exit status 1.")
    ...
    def test_download_and_install_native_plugin_zip_exception(self, mock_temporary_directory: Mock, mock_file: Mock, mock_validation_args: Mock, mock_execute: Mock, mock_get: Mock) -> None:
        mock_response = Mock()
        ...
        validate_tar = ValidateTar(mock_validation_args.return_value, mock_temporary_directory.return_value)
        with self.assertRaises(Exception) as e:
            validate_tar.download_and_install_native_plugin_zip(["analysis-icu", "analysis-nori"], os.path.join(mock_temporary_directory.return_value.path, "opensearch", "bin"))
        self.assertEqual(str(e.exception),
                         "Unable to download and install native plugin: Command './opensearch-plugin install --batch discovery-ec2', tmp/trytytyuit/bin' returned non-zero exit status 1.")

        mock_execute.assert_has_calls(
            [call('./opensearch-plugin install --batch file:/tmp/trytytyuit/opensearch/bin/analysis-icu-3.0.0.zip', '/tmp/trytytyuit/opensearch/bin')]
        )
    ...
    @patch('system.temporary_directory.TemporaryDirectory')
    @patch("builtins.open", new_callable=mock_open)
    def test_download_and_install_native_plugin_zip(self, mock_temporary_directory: Mock, mock_file: Mock,
                                                    mock_validation_args: Mock, mock_execute: Mock,
                                                    mock_get: Mock) -> None:
        mock_response = Mock()
        mock_validation_args.return_value.version = "3.0.0"
        mock_validation_args.os_build_number.return_value = "1123"
        ...
        mock_execute.side_effect = lambda *args, **kwargs: (0, "stdout_output", "stderr_output")
        validate_tar = ValidateTar(mock_validation_args.return_value, mock_temporary_directory.return_value)
        validate_tar.download_and_install_native_plugin_zip(["analysis-icu", "analysis-nori"],
                                                            os.path.join(mock_temporary_directory.return_value.path,
                                                                         "opensearch", "bin"))

        mock_execute.assert_has_calls(
            [
                call('./opensearch-plugin install --batch file:/tmp/trytytyuit/opensearch/bin/analysis-icu-3.0.0.zip',
                     '/tmp/trytytyuit/opensearch/bin'),
                call('./opensearch-plugin install --batch file:/tmp/trytytyuit/opensearch/bin/analysis-nori-3.0.0.zip',
                     '/tmp/trytytyuit/opensearch/bin')]
        )
